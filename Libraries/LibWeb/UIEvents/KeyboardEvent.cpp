...
static bool is_glyph_modifier_key(KeyCode platform_key)
{
...
static unsigned remove_non_glyph_modifier_keys(unsigned modifiers)
{
    return (modifiers & KeyModifier::Mod_Shift) | (modifiers & KeyModifier::Mod_Alt);
}
...
static ErrorOr<String> get_event_key(KeyCode platform_key, u32 code_point, unsigned modifiers)
{
    ...
    if (!is_glyph_modifier_key(platform_key)) {
        // 1. Set key to the key string that would have been generated by this event if it had been typed with all
        //    modifier keys removed except for glyph modifier keys.
        code_point = to_underlying(platform_key);
        modifiers = remove_non_glyph_modifier_keys(modifiers);

        if (is_ascii_upper_alpha(code_point) && !(modifiers & KeyModifier::Mod_Shift))
