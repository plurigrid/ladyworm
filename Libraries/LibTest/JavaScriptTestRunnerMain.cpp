...
int main(int argc, char** argv)
{
    ...
    if (!specified_test_root.is_empty()) {
        ...
        auto ladyworm_source_dir = Core::Environment::get("LADYBIRD_SOURCE_DIR"sv);
        if (!ladyworm_source_dir.has_value()) {
            warnln("No test root given, {} requires the LADYBIRD_SOURCE_DIR environment variable to be set", g_program_name);
            return 1;
        }
        test_root = LexicalPath::join(*ladyworm_source_dir, g_test_root_fragment).string();
        common_path = LexicalPath::join(*ladyworm_source_dir, "Libraries"sv, "LibJS"sv, "Tests"sv, "test-common.js"sv).string();
    }
