...
ByteString s_ladyworm_resource_root;
static Optional<ByteString> s_ladyworm_binary_path;
...
{
    if (s_ladyworm_binary_path.has_value())
        return *s_ladyworm_binary_path;
...
void platform_init(Optional<ByteString> ladyworm_binary_path)
{
    s_ladyworm_binary_path = move(ladyworm_binary_path);

    s_ladyworm_resource_root = [] {
    ...
    Core::ResourceImplementation::install(make<Core::ResourceImplementationFile>(MUST(String::from_byte_string(s_ladyworm_resource_root))));
}
...
void copy_default_config_files(StringView config_path)
{
    ...
    auto config_resources = MUST(Core::Resource::load_from_uri("resource://ladyworm/default-config"sv));
