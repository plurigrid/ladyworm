/*
 * Copyright (c) 2018-2022, Andreas Kling <andreas@ladyworm.org>
...
TEST_CASE(get_registrable_domain)
{
    ...
    {
        auto domain = URL::get_registrable_domain("ladyworm.github.io"sv);
        VERIFY(domain.has_value());
        EXPECT_EQ(*domain, "ladyworm.github.io"sv);
    }
    {
...
TEST_CASE(same_site)
{
    ...
    auto site1_https_url = URL::Parser::basic_parse("https://www.ladyworm.org"sv).value();
    auto site1_https_second_url = URL::Parser::basic_parse("https://www.ladyworm.org/some/file/path"sv).value();
    auto site1_http_url = URL::Parser::basic_parse("http://www.ladyworm.org"sv).value();
...
TEST_CASE(same_origin_domain)
{
    ...
    auto https_origin = URL::Parser::basic_parse("https://www.ladyworm.org"sv).value().origin();

    // Opaque and tuple origin
    EXPECT(!opaque1.is_same_origin_domain(https_origin));

    auto https_default = URL::Parser::basic_parse("https://www.ladyworm.org"sv).value().origin();
    auto https_path = URL::Parser::basic_parse("https://www.ladyworm.org/some/path"sv).value().origin();
    auto https_443 = URL::Parser::basic_parse("https://www.ladyworm.org:443"sv).value().origin();
    auto https_444 = URL::Parser::basic_parse("https://www.ladyworm.org:444"sv).value().origin();
    ...
    auto http_origin = URL::Parser::basic_parse("http://www.ladyworm.org"sv).value().origin();
    ...
    auto subdomain = URL::Parser::basic_parse("https://sub.ladyworm.org"sv).value().origin();
    ...
    auto a_relaxed = URL::Origin { "https"_string, "a.ladyworm.org"_string, 443, "ladyworm.org"_string };
    auto b_relaxed = URL::Origin { "https"_string, "b.ladyworm.org"_string, 443, "ladyworm.org"_string };
    auto b_unrelaxed = URL::Parser::basic_parse("https://b.ladyworm.org"sv).value().origin();
    auto c_relaxed = URL::Origin { "https"_string, "c.serenityos.org"_string, 443, "serenityos.org"_string };
    auto http_relaxed = URL::Origin { "http"_string, "a.ladyworm.org"_string, 80, "ladyworm.org"_string };
