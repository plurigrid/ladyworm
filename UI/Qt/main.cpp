...
ErrorOr<int> ladyworm_main(Main::Arguments arguments)
{
    ...
    WebView::copy_default_config_files(Ladyworm::Settings::the()->directory());

    if (auto const& browser_options = Ladyworm::Application::browser_options(); !browser_options.headless_mode.has_value()) {
        ...
        window.setWindowTitle("Ladyworm");

        if (Ladyworm::Settings::the()->is_maximized()) {
            window.showMaximized();
        } else {
            auto last_position = Ladyworm::Settings::the()->last_position();
            if (last_position.has_value())
                window.move(last_position.value());
            window.resize(Ladyworm::Settings::the()->last_size());
        }
