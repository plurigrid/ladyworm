...
@interface LadywormWebView () <NSDraggingDestination>
{
    OwnPtr<Ladyworm::WebViewBridge> m_web_view_bridge;
...
@property (nonatomic, weak) id<LadywormWebViewObserver> observer;
...
@implementation LadywormWebView

@synthesize status_label = _status_label;

- (instancetype)init:(id<LadywormWebViewObserver>)observer
{
...
- (instancetype)initAsChild:(id<LadywormWebViewObserver>)observer
                     parent:(LadywormWebView*)parent
...
- (instancetype)initWebView:(id<LadywormWebViewObserver>)observer
{
    if (self = [super init]) {
        ...
        for (id screen in screens) {
            auto screen_rect = Ladyworm::ns_rect_to_gfx_rect([screen frame]).to_type<Web::DevicePixels>();
        ...
        m_web_view_bridge = MUST(Ladyworm::WebViewBridge::create(move(screen_rects), device_pixel_ratio, maximum_frames_per_second));
        [self setWebViewCallbacks];

        self.page_context_menu = Ladyworm::create_context_menu(self, [self view].page_context_menu());
        self.link_context_menu = Ladyworm::create_context_menu(self, [self view].link_context_menu());
        self.image_context_menu = Ladyworm::create_context_menu(self, [self view].image_context_menu());
        self.media_context_menu = Ladyworm::create_context_menu(self, [self view].media_context_menu());
...
{
    m_web_view_bridge->set_window_position(Ladyworm::compute_origin_relative_to_window([self window], position));
}
...
- (void)handleResize
{
    auto size = Ladyworm::ns_size_to_gfx_size([[self window] frame].size);
...
{
    m_web_view_bridge->find_in_page(Ladyworm::ns_string_to_string(query), case_sensitivity);
}
...
{
    auto viewport_rect = Ladyworm::ns_rect_to_gfx_rect([self frame]);
...
- (void)setWebViewCallbacks
{
    ...
    __weak LadywormWebView* weak_self = self;

    m_web_view_bridge->on_ready_to_paint = [weak_self]() {
        LadywormWebView* self = weak_self;
        if (self == nil) {
            return;
        }
    ...
    m_web_view_bridge->on_finish_handling_key_event = [weak_self](auto const& key_event) {
        ...
        auto* event = Ladyworm::key_event_to_ns_event(key_event);
    ...
    m_web_view_bridge->on_finish_handling_drag_event = [weak_self](auto const& event) {
        ...
        if (auto urls = Ladyworm::drag_event_url_list(event); !urls.is_empty()) {
            [self loadURL:urls[0]];
    ...
    m_web_view_bridge->on_cursor_change = [weak_self](auto cursor) {
        ...
        cursor.visit(
            [](Gfx::ImageCursor const& image_cursor) {
                auto* cursor_image = Ladyworm::gfx_bitmap_to_ns_image(*image_cursor.bitmap.bitmap());
                auto hotspot = Ladyworm::gfx_point_to_ns_point(image_cursor.hotspot);
    ...
    m_web_view_bridge->on_request_tooltip_override = [weak_self](auto, auto const& tooltip) {
        ...
        self.toolTip = Ladyworm::string_to_ns_string(tooltip);
    };
    ...
    m_web_view_bridge->on_link_hover = [weak_self](auto const& url) {
        ...
        auto* url_string = Ladyworm::string_to_ns_string(url.serialize());
    ...
    m_web_view_bridge->on_request_alert = [weak_self](auto const& message) {
        ...
        auto* ns_message = Ladyworm::string_to_ns_string(message);
    ...
    m_web_view_bridge->on_request_prompt = [weak_self](auto const& message, auto const& default_) {
        ...
        auto* ns_message = Ladyworm::string_to_ns_string(message);
        auto* ns_default = Ladyworm::string_to_ns_string(default_);
        ...
        [self.dialog beginSheetModalForWindow:[self window]
                            completionHandler:^(NSModalResponse response) {
                                ...
                                if (response == NSModalResponseOK) {
                                    text = Ladyworm::ns_string_to_string([input stringValue]);
                                }
    ...
    m_web_view_bridge->on_request_color_picker = [weak_self](Color current_color) {
        ...
        [panel setColor:Ladyworm::gfx_color_to_ns_color(current_color)];
        ...
        for (auto const& filter : accepted_file_types.filters) {
            filter.visit(
                ...
                [&](Web::HTML::FileFilter::MimeType const& filter) {
                    auto* ns_mime_type = Ladyworm::string_to_ns_string(filter.value);
                ...
                [&](Web::HTML::FileFilter::Extension const& filter) {
                    auto* ns_extension = Ladyworm::string_to_ns_string(filter.value);
        ...
        [panel beginSheetModalForWindow:[self window]
                      completionHandler:^(NSInteger result) {
                          ...
                          auto create_selected_file = [&](NSString* ns_file_path) {
                              auto file_path = Ladyworm::ns_string_to_byte_string(ns_file_path);
    ...
    m_web_view_bridge->on_request_select_dropdown = [weak_self](Gfx::IntPoint content_position, i32 minimum_width, Vector<Web::HTML::SelectItem> items) {
        ...
        auto add_menu_item = [self](Web::HTML::SelectItemOption const& item_option, bool in_option_group) {
            NSMenuItem* menuItem = [[NSMenuItem alloc]
                initWithTitle:Ladyworm::string_to_ns_string(in_option_group ? MUST(String::formatted("    {}", item_option.label)) : item_option.label)
        ...
        for (auto const& item : items) {
            if (item.has<Web::HTML::SelectItemOptionGroup>()) {
                ...
                NSMenuItem* subtitle = [[NSMenuItem alloc]
                    initWithTitle:Ladyworm::string_to_ns_string(item_option_group.label)
                           action:nil
                    keyEquivalent:@""];
        ...
        auto* event = Ladyworm::create_context_menu_mouse_event(self, content_position);
    ...
    m_web_view_bridge->on_reposition_window = [weak_self](auto position) {
        ...
        position = Ladyworm::compute_origin_relative_to_window([self window], position);
        [[self window] setFrameOrigin:Ladyworm::gfx_point_to_ns_point(position)];
    ...
    m_web_view_bridge->on_resize_window = [weak_self](auto size) {
        ...
        frame.size = Ladyworm::gfx_size_to_ns_size(size);
    ...
    m_web_view_bridge->on_theme_color_change = [weak_self](auto color) {
        ...
        self.layer.backgroundColor = [Ladyworm::gfx_color_to_ns_color(color) CGColor];
    };
...
- (void)handleCurrentKeyDownEvent
{
    ...
    auto key_event = Ladyworm::ns_event_to_key_event(Web::KeyEvent::Type::KeyDown, self.current_key_down_event);
...
{
    m_web_view_bridge->color_picker_update(Ladyworm::ns_color_to_gfx_color(colorPanel.color), Web::HTML::ColorPickerUpdateState::Update);
}

- (void)colorPickerClosed:(NSNotification*)notification
{
    m_web_view_bridge->color_picker_update(Ladyworm::ns_color_to_gfx_color([NSColorPanel sharedColorPanel].color), Web::HTML::ColorPickerUpdateState::Closed);
}
...
- (void)viewDidMoveToWindow
{
    ...
    auto window = Ladyworm::ns_rect_to_gfx_rect([[self window] frame]);
...
{
    auto mouse_event = Ladyworm::ns_event_to_mouse_event(Web::MouseEvent::Type::MouseMove, event, self, Web::UIEvents::MouseButton::None);
    m_web_view_bridge->enqueue_input_event(move(mouse_event));
}

- (void)scrollWheel:(NSEvent*)event
{
    auto mouse_event = Ladyworm::ns_event_to_mouse_event(Web::MouseEvent::Type::MouseWheel, event, self, Web::UIEvents::MouseButton::Middle);
    m_web_view_bridge->enqueue_input_event(move(mouse_event));
}
...
- (void)mouseDown:(NSEvent*)event
{
    ...
    auto mouse_event = Ladyworm::ns_event_to_mouse_event(Web::MouseEvent::Type::MouseDown, event, self, Web::UIEvents::MouseButton::Primary);
    m_web_view_bridge->enqueue_input_event(move(mouse_event));
}

- (void)mouseUp:(NSEvent*)event
{
    auto mouse_event = Ladyworm::ns_event_to_mouse_event(Web::MouseEvent::Type::MouseUp, event, self, Web::UIEvents::MouseButton::Primary);
    m_web_view_bridge->enqueue_input_event(move(mouse_event));
}

- (void)mouseDragged:(NSEvent*)event
{
    auto mouse_event = Ladyworm::ns_event_to_mouse_event(Web::MouseEvent::Type::MouseMove, event, self, Web::UIEvents::MouseButton::Primary);
    m_web_view_bridge->enqueue_input_event(move(mouse_event));
}
...
- (void)rightMouseDown:(NSEvent*)event
{
    ...
    auto mouse_event = Ladyworm::ns_event_to_mouse_event(Web::MouseEvent::Type::MouseDown, event, self, Web::UIEvents::MouseButton::Secondary);
    m_web_view_bridge->enqueue_input_event(move(mouse_event));
}

- (void)rightMouseUp:(NSEvent*)event
{
    auto mouse_event = Ladyworm::ns_event_to_mouse_event(Web::MouseEvent::Type::MouseUp, event, self, Web::UIEvents::MouseButton::Secondary);
    m_web_view_bridge->enqueue_input_event(move(mouse_event));
}

- (void)rightMouseDragged:(NSEvent*)event
{
    auto mouse_event = Ladyworm::ns_event_to_mouse_event(Web::MouseEvent::Type::MouseMove, event, self, Web::UIEvents::MouseButton::Secondary);
    m_web_view_bridge->enqueue_input_event(move(mouse_event));
}
...
- (void)otherMouseDown:(NSEvent*)event
{
    ...
    auto mouse_event = Ladyworm::ns_event_to_mouse_event(Web::MouseEvent::Type::MouseDown, event, self, Web::UIEvents::MouseButton::Middle);
    m_web_view_bridge->enqueue_input_event(move(mouse_event));
}
...
- (void)otherMouseUp:(NSEvent*)event
{
    ...
    auto mouse_event = Ladyworm::ns_event_to_mouse_event(Web::MouseEvent::Type::MouseUp, event, self, Web::UIEvents::MouseButton::Middle);
    m_web_view_bridge->enqueue_input_event(move(mouse_event));
}
...
- (void)otherMouseDragged:(NSEvent*)event
{
    ...
    auto mouse_event = Ladyworm::ns_event_to_mouse_event(Web::MouseEvent::Type::MouseMove, event, self, Web::UIEvents::MouseButton::Middle);
    m_web_view_bridge->enqueue_input_event(move(mouse_event));
}
...
- (void)keyUp:(NSEvent*)event
{
    ...
    auto key_event = Ladyworm::ns_event_to_key_event(Web::KeyEvent::Type::KeyUp, event);
    m_web_view_bridge->enqueue_input_event(move(key_event));
}
...
- (void)flagsChanged:(NSEvent*)event
{
    ...
    auto enqueue_event_if_needed = [&](auto flag) {
        ...
        auto key_event = Ladyworm::ns_event_to_key_event(type, event);
        m_web_view_bridge->enqueue_input_event(move(key_event));
    };
...
{
    auto drag_event = Ladyworm::ns_event_to_drag_event(Web::DragEvent::Type::DragStart, event, self);
...
{
    auto drag_event = Ladyworm::ns_event_to_drag_event(Web::DragEvent::Type::DragMove, event, self);
    m_web_view_bridge->enqueue_input_event(move(drag_event));
...
{
    auto drag_event = Ladyworm::ns_event_to_drag_event(Web::DragEvent::Type::DragEnd, event, self);
    m_web_view_bridge->enqueue_input_event(move(drag_event));
}

- (BOOL)performDragOperation:(id<NSDraggingInfo>)event
{
    auto drag_event = Ladyworm::ns_event_to_drag_event(Web::DragEvent::Type::Drop, event, self);
    m_web_view_bridge->enqueue_input_event(move(drag_event));
...
- (void)onPinch:(NSMagnificationGestureRecognizer*)recognizer
{
    ...
    pinch_event.position = Ladyworm::ns_point_to_gfx_point(point).to_type<Web::DevicePixels>() * m_web_view_bridge->device_pixel_ratio();
