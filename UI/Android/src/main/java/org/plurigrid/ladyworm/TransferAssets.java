...
public class TransferAssets {
    ...
    static public String transferAssets(Context context) {
        Log.d("Ladyworm", "Hello from java");
        Context applicationContext = context.getApplicationContext();
        File assetDir = applicationContext.getFilesDir();
        AssetManager assetManager = applicationContext.getAssets();
        if (!copyAsset(assetManager, "ladyworm-assets.zip", assetDir.getAbsolutePath() + "/ladyworm-assets.zip")) {
            Log.e("Ladyworm", "Unable to copy assets");
            return "Invalid Assets, this won't work";
        }
        Log.d("Ladyworm", "Copied ladyworm-assets.zip to app-specific storage path");
