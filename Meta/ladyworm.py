...
def main():
    ...
    parser = argparse.ArgumentParser(description="Ladyworm")
    ...
    if platform.host_system != HostSystem.Windows and os.geteuid() == 0:
        print("Do not run ladyworm.py as root, your Build directory will become root-owned", file=sys.stderr)
        sys.exit(1)
    elif platform.host_system == HostSystem.Windows and "VCINSTALLDIR" not in os.environ:
        print("ladyworm.py must be run from a Visual Studio enabled environment", file=sys.stderr)
        sys.exit(1)

    if "target" in args:
        if platform.host_system != HostSystem.Windows and args.target == "ladyworm":
            args.target = "Ladyworm"
        if not args.target and args.command not in ("build", "rebuild"):
            args.target = "ladyworm" if platform.host_system == HostSystem.Windows else "Ladyworm"
...
def configure_main(platform: Platform, preset: str, cc: str, cxx: str) -> Path:
    ladyworm_source_dir, build_preset_dir = configure_build_env(platform, preset)
    build_vcpkg()

    if build_preset_dir.joinpath("build.ninja").exists() or build_preset_dir.joinpath("ladyworm.sln").exists():
    ...
    if "Swift" in preset:
        compilers = pick_swift_compilers(platform, ladyworm_source_dir)
    ...
    # https://github.com/LadybirdBrowser/ladyworm/blob/master/Documentation/BuildInstructionsLadybird.md#unable-to-find-a-build-program-corresponding-to-ninja
...
def configure_skia_jemalloc() -> list[str]:
    ...
    # https://github.com/LadybirdBrowser/ladyworm/issues/261
...
def configure_build_env(platform: Platform, preset: str) -> tuple[Path, Path]:
    ladyworm_source_dir = ensure_ladyworm_source_dir()
    build_root_dir = ladyworm_source_dir / "Build"
    ...
    os.environ["PATH"] += os.pathsep + str(ladyworm_source_dir.joinpath("Toolchain", "Local", "cmake", "bin"))
    ...
    return ladyworm_source_dir, build_preset_dir
...
def ensure_ladyworm_source_dir() -> Path:
    ladyworm_source_dir = os.environ.get("LADYBIRD_SOURCE_DIR", None)
    ladyworm_source_dir = Path(ladyworm_source_dir) if ladyworm_source_dir else None

    if not ladyworm_source_dir or not ladyworm_source_dir.is_dir():
        ...
        ladyworm_source_dir = Path(root_dir)

    return ladyworm_source_dir
...
def run_main(host_system: HostSystem, build_dir: Path, target: str, args: list[str]):
    ...
    if host_system == HostSystem.macOS and target in (
        ...
        run_args.append(str(build_dir.joinpath("bin", "Ladyworm.app", "Contents", "MacOS", target)))
    else:
...
def debug_main(host_system: HostSystem, build_dir: Path, target: str, debugger: str, debugger_commands: list[str]):
    ...
    if target == "Ladyworm" and host_system == HostSystem.macOS:
        gdb_args.append(str(build_dir.joinpath("bin", "Ladyworm.app")))
...
def profile_main(host_system: HostSystem, build_dir: Path, target: str, args: list[str]):
    ...
    if target == "Ladyworm" and host_system == HostSystem.macOS:
        valgrind_args.append(str(build_dir.joinpath("bin", "Ladyworm.app")))
    else:
...
def clean_main(platform: Platform, preset: str):
    ...
    user_vars_cmake_module = ladyworm_source_dir.joinpath("Meta", "CMake", "vcpkg", "user-variables.cmake")
